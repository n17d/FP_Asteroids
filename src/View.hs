...
view GameState{player = p@Player{rotation = rot, direction = dir, lives = hp}, bullets = bs, time = steps, state = st, enemies = e, asteroids = as}
        ...
        | otherwise = return $ scale 10 10 $ pic
          where pic = pictures(drawPlayer p ++ rotation ++ directionDegrees ++ concatMap drawBullet bs ++ showTime ++ concatMap drawEnemy e ++ concatMap drawAsteroid as ++ asteroidAmount ++ showLives)
