...
view GameState{player = p@Player{rotation = rot, direction = dir}, bullets = bs, time = steps, state = st}
        ...
        | otherwise = return $ scale 10 10 $ pic
          where pic = pictures(drawPlayer p ++ rotation ++ directionDegrees ++ concatMap drawBullet bs ++ showTime)
